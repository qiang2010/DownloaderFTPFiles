...
public class FTPFilesDownloader { 
	...
	public FTPFilesDownloader()    {
		...
		InputStream in = getClass().getResourceAsStream("ftp.properties");
		try {
			pro.load(in);
		} catch (IOException e) {
			e.printStackTrace();
...
//	private long deleteInterval = 10*60*60*1000;
	...
	public boolean downloadFiles(String ftpPaht,String localPath) throws ConnectException{

		TimeZone.setDefault(TimeZone.getTimeZone("GSM+8"));
		long timeContion = new Date().getTime() - timeInterval*2;
		...
		try {
			...
			for(String  curName:fileNames){
				...
				if(fileTime < timeContion && tempLocalFile.exists()) {
					// 删除文件,五个个小时以前，并且本地已经存在的文件删除。
					//if(fileTime < timeContion-timeInterval){
						System.out.println("delete: "+curName);
